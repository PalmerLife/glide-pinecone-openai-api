...
def jw_link(book, chapter, verse):
    ...
    verse_str = str(int(verse)).zfill(3)
...
async def ask_question(request: Request, x_api_key: str = Header(None)):
    ...
    for i, b in enumerate(bullets):
        if i < len(query_response["matches"]):
            ...
            stored_link = meta.get("jworg_link", "")

            print(f"🔎 Meta {i+1}: book={book}, chapter={chapter}, verse={verse}, link={stored_link}")
            ...
            link = stored_link or jw_link(book, chapter, verse)
        else:
            ref = ""
            link = ""
